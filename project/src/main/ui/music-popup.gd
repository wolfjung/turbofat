...
func _ready() -> void:
	MusicPlayer.connect("current_track_changed", Callable(self, "_on_MusicPlayer_current_track_changed"))
...
func _refresh_panel(value: CheckpointMusicTrack) -> void:
	...
	if _shown_track:
		...
		_music_panel.get("theme_override_styles/panel").set("bg_color", _shown_track.track_color)
...
func _on_Label_item_rect_changed() -> void:
	...
	_music_panel.position.x = get_viewport().get_visible_rect().size.x / 2 - _music_panel.size.x / 2

...
func _refresh() -> void:
	...
	if has_focus():
		_panel.get("theme_override_styles/panel").set_border_color(FOCUSED_BORDER_COLOR)
	elif is_hovered():
		_panel.get("theme_override_styles/panel").set_border_color(HOVERED_BORDER_COLOR)
	else:
		_panel.get("theme_override_styles/panel").set_border_color(DEFAULT_BORDER_COLOR)
	_panel.get("theme_override_styles/panel").set_bg_color(color)

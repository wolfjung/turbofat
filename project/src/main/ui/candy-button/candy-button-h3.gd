...
@export var icon_left: Texture2D: set = set_icon_left

## Icon shown to the right of the button's text.
@export var icon_right: Texture2D: set = set_icon_right
...
func set_icon_left(new_icon_left: Texture2D) -> void:
	icon_left = new_icon_left
	_refresh_icons()
	_refresh_font_size()


func set_icon_right(new_icon_right: Texture2D) -> void:
...
func _refresh_font_size() -> void:
	...
	_label.set("theme_override_fonts/font", chosen_font.duplicate())
	_refresh_label_color()
...
func _refresh_icon_color() -> void:
	...
	_icon_node_left.material.set_shader_parameter("black", _gradient_helper.gradient.sample(
...
func _refresh_label_color() -> void:
	_label.get_font("font").outline_color = _gradient_helper.gradient.sample(
			0.25 if has_focus() else 0.15)

...
var creature_color: Color: set = set_creature_color

## Virtual property; value is only exposed through getters/setters
var color_presets := []: set = set_color_presets
...
func _pressed() -> void:
	...
	popup_position += Vector2(-_popup.size.x, 0)
	var screen_size := get_viewport_rect().size
	popup_position.x = clamp(popup_position.x, 0, screen_size.x - _popup.size.x)
	popup_position.y = clamp(popup_position.y, 0, screen_size.y - _popup.size.y)
	
	_popup.popup(Rect2(popup_position, _popup.size))
...
func _refresh_color() -> void:
	...
	material.get_shader_parameter("gradient").gradient = _gradient()
...
func _refresh_creature_color() -> void:
	...
	_icon.modulate = _gradient().sample(0.3) if disabled else creature_color

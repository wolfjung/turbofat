...
@export var obstacles_path: NodePath: set = set_obstacles_path
...
func _refresh_obstacles_path() -> void:
	...
	if obstacles_path:
		...
		_creature_shadows.notify_property_list_changed()
	...
	if obstacles_path and get_node(obstacles_path).has_node("ObstacleMap"):
		...
		_obstacle_map_shadows.notify_property_list_changed()

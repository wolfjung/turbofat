...
@export var visual_fatness := 1.0: set = set_visual_fatness

## how fat the creature should be; 5.0 = 5x normal size
var fatness := 1.0: set = set_fatness
...
func _ready() -> void:
	...
	if not Engine.is_editor_hint():
		...
		connect("dna_loaded", Callable($Animations/IdleTimer, "_on_CreatureVisuals_dna_loaded"))
